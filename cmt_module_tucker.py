...
class LMHSA(nn.Module):
    ...
    def __init__(self, input_size, channels, d_k, d_v, stride, heads, dropout):
        ...
        self.B = nn.Parameter(torch.Tensor(1, self.heads, input_size ** 2, (input_size // stride) ** 2),
                              requires_grad=True)
...
class CMTStem(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, ch_com_rate=0.5, kernel_size=3, compress_size=3, affine=True,
                 group=True):
        super().__init__()
        self.conv1 = TuckerLayer(in_channels, out_channels, ch_com_rate=ch_com_rate, kernel_size=kernel_size,
                                 compress_size=compress_size, stride=2, affine=affine, group=group)
        # self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=2, padding=1, bias=False)
        self.gelu1 = nn.GELU()
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.conv2 = TuckerLayer(out_channels, out_channels, ch_com_rate=ch_com_rate, kernel_size=kernel_size,
                                 compress_size=compress_size, stride=1, affine=affine, group=group)
        # self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False)
        self.gelu2 = nn.GELU()
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.conv3 = TuckerLayer(out_channels, out_channels, ch_com_rate=ch_com_rate, kernel_size=kernel_size,
                                 compress_size=compress_size, stride=1, affine=affine, group=group)
